...
interface CreatePermitModalProps {
  ...
  currentUser: any; // 🟢 รับ currentUser เข้ามาเพื่อเช็ค Role
}
...
export default function CreatePermitModal({ open, onCancel, onSubmit, isSubmitting, currentUser }: CreatePermitModalProps) {
  ...
  // 🟢 เช็คว่า User คนนี้เป็น จป. หรือ Admin หรือไม่
  ...
  const handleFinish = async () => {
    // 🟢 ดักไว้ชั้นที่ 2 เผื่อหลุดเข้ามา
    if (isSafetyOrAdmin) {
  ...
  return (
    ...
    >
      ...
      {/* 🟢 หน้าจอเตือน กรณีเป็น จป. หรือ Admin พยายามจะมาขอ Permit */}
      {isSafetyOrAdmin ? (
        <div className="flex-1 flex flex-col items-center justify-center p-8 bg-slate-50 rounded-xl md:rounded-[1.5rem] relative overflow-hidden">
          ...
          <p className="text-slate-500 text-center font-medium text-sm md:text-base max-w-md">
            บทบาท <b>{currentUser?.role === 'SAFETY_ENGINEER' ? 'เจ้าหน้าที่ความปลอดภัย (จป.)' : 'ผู้ดูแลระบบ (Admin)'}</b> ถูกกำหนดให้เป็นผู้อนุมัติและตรวจสอบเท่านั้น<br/><br/>
            หากต้องการขอใบอนุญาต กรุณาใช้บัญชีของผู้รับเหมาหรือพนักงานระดับปฏิบัติการ (EMPLOYEE)
          </p>
          <Button size="large" type="primary" danger onClick={handleClose} className="mt-8 font-black px-8 rounded-xl h-12 shadow-lg hover:scale-105 transition-transform">
            ปิดหน้าต่างนี้
          </Button>
        </div>
      ) : (
        /* 🟢 หน้าจอปกติ สำหรับพนักงานทั่วไป (EMPLOYEE) หรือ AREA_OWNER */
        <>
