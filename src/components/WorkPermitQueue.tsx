...
export default function WorkPermitQueue({ permits, loading, currentUser, onPreviewFile, onViewDetails, onUpdateStatus }: any) {
  ...
  // 🧪 ซิงค์ API บันทึกก๊าซกลับไปที่ Cloud
  const handleSubmitGasLog = async (values: any) => {
    ...
    } catch (error) { message.error('ไม่สามารถบันทึกข้อมูลได้ กรุณาลองใหม่อีกครั้ง'); } finally { setIsSubmittingGas(false); }
  };
  ...
  const handleSubmitExtend = async (values: any) => {
    ...
    try {
      ...
      setTimeout(() => window.location.reload(), 1000); // โหลดหน้าใหม่เพื่ออัปเดตเวลา
    } catch (error) { message.error('ระบบขัดข้อง ไม่สามารถขยายเวลาได้'); } finally { setIsSubmittingExtend(false); }
  };

  const columns: ColumnsType<any> = [
    { 
      title: 'Permit No.', dataIndex: 'permit_number', key: 'permit_number', width: 130, 
      render: (text) => (<span className="inline-block bg-slate-100 text-slate-700 font-mono font-bold px-2.5 py-1 rounded-lg border border-slate-200 shadow-sm text-sm whitespace-nowrap">{text || 'PTW-XX'}</span>)
    },
    { 
      title: 'รายละเอียดงาน (Work Details)', key: 'details', 
      render: (_, record) => ( 
        <div className="flex flex-col gap-2 min-w-[280px] py-2">
          <div className="font-bold text-slate-800 text-base leading-tight tracking-tight">{record.title}</div>
          <div className="flex flex-wrap items-center gap-3 text-xs text-slate-500">
            <div className="flex items-center gap-1.5 font-medium bg-slate-50 px-2 py-1 rounded-md border border-slate-100"><Avatar size="small" icon={<UserOutlined />} className="bg-blue-100 text-blue-600 w-5 h-5 flex items-center justify-center text-[10px]" /> <span className="text-slate-700">{record.applicant?.full_name || 'ไม่ทราบชื่อ'}</span></div>
            <div className="flex items-center gap-1 font-medium bg-slate-50 px-2 py-1 rounded-md border border-slate-100"><EnvironmentOutlined className="text-emerald-500" /> <span className="text-slate-600 truncate max-w-[150px] sm:max-w-xs">{record.location_detail}</span></div>
          </div>
          {record.attachment_url && (<button onClick={() => onPreviewFile(record.attachment_url)} className="mt-1 flex items-center justify-center gap-1.5 text-xs font-semibold text-indigo-600 bg-indigo-50/50 hover:bg-indigo-100 border border-indigo-200 px-3 py-1.5 rounded-lg transition-all duration-200 w-fit group"><FileTextOutlined className="group-hover:scale-110 transition-transform" /> ดูเอกสาร JSA</button>)}
        </div> 
      ) 
    },
    ...
    { 
      title: 'การจัดการ (Action)', key: 'action', width: 190, 
      render: (_, record) => {
        ...
        return (
          <div className="flex flex-col gap-2 w-full pr-2">
            <button onClick={() => onViewDetails(record)} className="w-full flex items-center justify-center gap-1.5 px-3 py-1.5 bg-white border border-slate-300 text-slate-700 rounded-lg text-xs font-bold hover:bg-slate-50 hover:text-blue-600 hover:border-blue-300 transition-all whitespace-nowrap shadow-sm"><EyeOutlined /> ดูรายละเอียด</button>

            {(isAreaOwnerTurn || isSafetyTurn) && ( 
              <div className="flex gap-2 w-full">
                <button onClick={() => onUpdateStatus(record.id, record.status, 'APPROVE')} className="flex-1 flex items-center justify-center gap-1 px-2 py-1.5 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-lg text-xs font-bold shadow-md hover:shadow-lg hover:from-emerald-400 hover:to-teal-400 active:scale-95 transition-all whitespace-nowrap"><CheckOutlined /> อนุมัติ</button>
                <button onClick={() => onUpdateStatus(record.id, record.status, 'REJECT')} className="w-8 h-8 flex-shrink-0 flex items-center justify-center bg-white text-rose-500 rounded-lg border border-rose-200 hover:bg-rose-500 hover:text-white hover:border-rose-500 shadow-sm active:scale-95 transition-all"><CloseOutlined /></button>
              </div> 
            )}
            ...
            {isApproved && (
              <div className="flex flex-col gap-2 w-full mt-1 border-t border-slate-100 pt-2">
                ...
                {(() => {
                  ...
                  if (hasGasLog) {
                    return <div className="w-full flex items-center justify-center gap-1.5 px-2 py-1.5 bg-emerald-50 text-emerald-600 rounded-lg text-[11px] font-bold border border-emerald-200 select-none shadow-sm cursor-default"><CheckCircleOutlined /> ตรวจก๊าซเรียบร้อยแล้ว</div>;
                  } else {
                    return <button onClick={() => handleOpenGasModal(record)} className="w-full flex items-center justify-center gap-1.5 px-2 py-1.5 bg-cyan-50 hover:bg-cyan-500 text-cyan-700 hover:text-white rounded-lg text-[11px] font-bold transition-all border border-cyan-200 hover:border-cyan-500 shadow-sm"><DashboardOutlined /> ตรวจก๊าซ & Safety Talk</button>;
                  }
                })()}

                {/* ⏳ ผู้รับเหมา: ขอต่อเวลา */}
                {isApplicant && (
                  <button onClick={() => handleOpenExtendModal(record)} className="w-full flex items-center justify-center gap-1.5 px-2 py-1.5 bg-purple-50 hover:bg-purple-500 text-purple-700 hover:text-white rounded-lg text-[11px] font-bold transition-all border border-purple-200 hover:border-purple-500 shadow-sm">
                ...
                {isApplicant && (
                  <Popconfirm title="ยืนยันการปิดงาน?" description="คุณได้ทำความสะอาดพื้นที่เรียบร้อยแล้วใช่หรือไม่?" onConfirm={() => onUpdateStatus(record.id, record.status, 'CLOSE')} okText="ยืนยันปิดงาน" cancelText="ยกเลิก">
                    <button className="w-full flex items-center justify-center gap-1.5 px-2 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg text-[11px] font-bold transition-all border border-slate-200"><CheckCircleOutlined /> ปิดงาน/คืนพื้นที่</button>
                  </Popconfirm>
                )}
                ...
                {isOwnerOrSafety && (
                  <Popconfirm title="สั่งระงับงานฉุกเฉิน!" description="ต้องการยกเลิกใบอนุญาตนี้ทันทีใช่หรือไม่?" onConfirm={() => onUpdateStatus(record.id, record.status, 'REVOKE')} okText="ระงับงาน" okButtonProps={{danger: true}} cancelText="ยกเลิก">
                    <button className="w-full flex items-center justify-center gap-1.5 px-2 py-1.5 bg-red-50 hover:bg-red-500 text-red-600 hover:text-white rounded-lg text-[11px] font-bold transition-all border border-red-200 hover:border-red-500"><StopOutlined /> สั่งระงับงาน (Revoke)</button>
                  </Popconfirm>
                )}
              </div>
            )}
  ...
  return (
    <>
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <Table columns={columns} dataSource={permits} loading={loading} pagination={{ pageSize: 8, className: "px-4 pb-4" }} size="middle" scroll={{ x: 1000 }} className="modern-table" />
      </div>

      {/* 🧪 Modal บันทึกก๊าซ */}
      <Modal title={null} open={isGasModalOpen} onCancel={() => setIsGasModalOpen(false)} footer={null} width={600} centered styles={{ body: { padding: 0 } }} destroyOnClose>
        <div className="bg-gradient-to-r from-cyan-600 to-blue-600 p-6 rounded-t-xl text-white shadow-sm relative overflow-hidden">
          ...
          <h2 className="text-xl md:text-2xl font-bold m-0 flex items-center gap-3 text-white relative z-10"><div className="bg-white/20 p-2 rounded-lg"><DashboardOutlined /></div>บันทึกผลตรวจวัดก๊าซหน้างาน</h2>
          <p className="text-cyan-100 text-xs md:text-sm mt-2 opacity-90 mb-0 relative z-10">Permit No: <span className="font-mono font-bold bg-black/20 px-2 py-0.5 rounded">{selectedGasPermit?.permit_number}</span></p>
        </div>
        <div className="p-4 md:p-6 bg-slate-50">
          <Form form={gasForm} layout="vertical" onFinish={handleSubmitGasLog} requiredMark={false}>
            <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 mb-5 flex items-start gap-3">
              ...
              <div className="flex-1">
                <Form.Item name="safety_talk" valuePropName="checked" rules={[{ validator: (_, value) => value ? Promise.resolve() : Promise.reject(new Error('ต้องทำการ Safety Talk ก่อนเริ่มงาน')) }]} className="m-0">
                  <Checkbox className="font-bold text-slate-800 text-sm md:text-base">ยืนยันการทำ Safety Talk (Toolbox Talk)</Checkbox>
                </Form.Item>
            ...
            <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 mb-6">
              <h4 className="font-bold text-slate-700 mb-4 flex items-center gap-2 border-b border-slate-100 pb-2"><DashboardOutlined className="text-blue-500" /> ค่ามาตรฐานก๊าซ (Atmospheric Testing)</h4>
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-50 p-3 rounded-xl border border-slate-200"><Form.Item name="o2" label={<span className="font-bold text-slate-600 text-xs">Oxygen (O₂) <span className="text-emerald-500 font-normal ml-1">19.5 - 23.5%</span></span>} rules={[{ required: true, message: 'ระบุค่า O2' }]} className="m-0"><InputNumber size="large" className="w-full text-lg font-mono font-bold text-blue-600" placeholder="0.0" suffix="%" step={0.1} /></Form.Item></div>
                <div className="bg-slate-50 p-3 rounded-xl border border-slate-200"><Form.Item name="lel" label={<span className="font-bold text-slate-600 text-xs">ก๊าซไวไฟ (LEL) <span className="text-emerald-500 font-normal ml-1">&lt; 10%</span></span>} rules={[{ required: true, message: 'ระบุค่า LEL' }]} className="m-0"><InputNumber size="large" className="w-full text-lg font-mono font-bold text-orange-500" placeholder="0.0" suffix="%" step={0.1} /></Form.Item></div>
                <div className="bg-slate-50 p-3 rounded-xl border border-slate-200"><Form.Item name="h2s" label={<span className="font-bold text-slate-600 text-xs">ก๊าซไข่เน่า (H₂S) <span className="text-emerald-500 font-normal ml-1">&lt; 10 ppm</span></span>} rules={[{ required: true, message: 'ระบุค่า H2S' }]} className="m-0"><InputNumber size="large" className="w-full text-lg font-mono font-bold text-purple-600" placeholder="0.0" suffix="ppm" step={0.1} /></Form.Item></div>
                <div className="bg-slate-50 p-3 rounded-xl border border-slate-200"><Form.Item name="co" label={<span className="font-bold text-slate-600 text-xs">ก๊าซพิษ (CO) <span className="text-emerald-500 font-normal ml-1">&lt; 25 ppm</span></span>} rules={[{ required: true, message: 'ระบุค่า CO' }]} className="m-0"><InputNumber size="large" className="w-full text-lg font-mono font-bold text-rose-500" placeholder="0.0" suffix="ppm" step={0.1} /></Form.Item></div>
              </div>
            </div>
      ...
      <Modal title={null} open={isExtendModalOpen} onCancel={() => setIsExtendModalOpen(false)} footer={null} width={500} centered styles={{ body: { padding: 0 } }} destroyOnClose>
